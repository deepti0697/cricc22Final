...
//        aTeamNameLabel.isHidden = true
    ...
    fileprivate func updateTeamName() {
        ...
        if let heroObject = UserDefaults.standard.value(forKey: "loginData") as? NSData {
            ...
            let parameters = ["user_id": UserDic.value(forKey: "user_id") ?? "","teamname": aTeamNameTextField.text ?? "","state": state.text ?? "", "user_refer_code": referCode.text ?? ""] as [String : Any]
...
//                         self.teamNameClouser(self.aTeamNameTextField.text ?? "")
                        Loader.showToast(message: jsonMessage, onView: self.view, bottomMargin: 200, color: UIColor(red: 49, green: 139, blue: 22))
                                                            self.dismissVC()
                    }
                    else {
