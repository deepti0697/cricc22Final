...
class VerifyMobileNumberViewController: BaseClass {
    ...
    func setUp(){
        ...
        if mobileNumber != "" {
            ...
            mobileNolbl.text = storeMobileData ?? ""
        }
        didntReciveOtpLbl.addLine(position: .LINE_POSITION_BOTTOM, color: UIColor.black, width: 1)
        ...
        mobileNolbl.addLine(position: .LINE_POSITION_BOTTOM, color:UIColor.FlatColor.Black.balckcolor, width: 1)
//        continueToNext.backgroundColor = UIColor.FlatColor.Blue.GreenishBlue
//        continueToNext.setTitleColor(UIColor.FlatColor.white.whitecolor, for: .normal)
    }
            ...
            Alamofire.request(urlString, method: .post, parameters: param, encoding: JSONEncoding.default, headers: headers).responseJSON{ response in
                ...
                if json != nil{
                    ...
                    if status == 1 { // for Success
                        ...
                        let kuser = UserDic.value(forKey: "user_id")
