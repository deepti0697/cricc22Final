...
class NotificationViewController: BaseClass {
    ...
    override func viewWillAppear(_ animated: Bool) {
        self.addNavigationBar(navigationTitle: "Your Notification", titleContentMode: .left, isNavImageShow: false, isBackButtonShow: true, isWalletButtonShow: false, isBinButtonShow: false)
        hitNotification()
    }
    ...
    func hitNotification() {
        ...
        if let heroObject = UserDefaults.standard.value(forKey: "loginData") as? NSData {
            ...
            self.hideActivityIndicator()
                do {
                    let jsonRespnse = try JSON(data:response as! Data)
